...
因为不想在本地安装Docker，考虑在本地用Docker Client 远程连接到远程服务器的Docker Daemon 上。
...
Docker client，Docker 的客户端，可以用来控制 Daemon 运行Docker 项目。

## 服务端（Debian）配置远程连接

覆盖配置docker.service 即可
...
\-H tcp://0.0.0.0:2337  
监听2337端口，使得 Daemon 可以由2337端口远程访问  
  
\-H unix:///var/run/docker.sock  
监听 Unix 域Socket，使得原本的本地连接方式依然可用
...
至此，我们就能通过2337端口远程访问Docker Daemon 了
...
临时指定 Docker 服务器的地址  
需要修改x.x.x.x 为服务器的ip  
如有需要，可以把DOCKER\_HOST 环境变量写入.bashrc 或者.zshrc
...
发现远程连接不需要验证，如果有人获取了我们服务器的 ip 与端口，他就能直接连接到Docker Daemon。并且 Docker 具有root 权限，不加密十分危险。
...
首先拷贝密钥  
将服务器上/etc/docker/certs 下的ca.pem、cert.pem、key.pem 拷贝到客户端的~/.docker 中
