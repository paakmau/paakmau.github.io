...
按 F5 直接运行，VSC 自动生成 `launch.json`，有时候会失败，可以重试几遍<br>
内容大概如下
...
解释：
它配置了两个 launch，一个是以当前打开的 Java 源文件作为主类调试，一个是以 `Hello.java` 作为主类进行调试。

切回 `Hello.java` 按 `F5`，就可以开始调试了。
