...
上面代码中操作 shared\_ptr 的原语内部可能不是无锁的  
因为 shared\_ptr 的内部存了两个指针，而目前在硬件层面上对两个指针进行 CAS 操作还不太能实现（据说 TSX 指令集支持但是效率不高）

另外操作 shared\_ptr 的 atomic 模版方法在 C++20 中已经弃用，以后应当用 atomic 模板类（然而国内普及 C++20 可能要等到 2200 年）
