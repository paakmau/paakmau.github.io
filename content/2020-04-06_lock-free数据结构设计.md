...
读的时候 xjb 读  
写的时候生成一个新的head，然后用 CAS 不停的尝试把当前的head 替换掉，同时注意保证新的 head 指向当前的head
...
上面代码中操作shared\_ptr 的原语内部可能不是无锁的  
因为shared\_ptr 的内部存了两个指针，而目前在硬件层面上对两个指针进行 CAS 操作还不太能实现（据说 TSX 指令集支持但是效率不高）

另外操作shared\_ptr 的 atomic 模版方法在C++20中已经弃用，以后应当用 atomic 模板类（然而国内普及C++20可能要等到2200年）
