...
稍微麻烦一点，可以用 MinGW-w64编译 GLFW 的源码并安装，最后添加环境变量
...
最后需要添加一个 glfw3\_DIR 环境变量，指向存放配置 GLFW 的 CMake 模块的路径，值是 C:\\Program Files (x86)\\GLFW\\lib\\cmake\\glfw3，如果你使用了不同的安装路径，请做相应修改，这样后续 find\_package 才能找到 glfw3的头文件和依赖库
...
简单说就是这个项目只有 main.cpp 一个文件，然后寻找 Vulkan 和 glfw3这两个库，并链接到我们的项目中
