...
前文和手册中都有介绍过 `CollisionFilter`，但是这个东西是跟 `Collider` 绑在一起扔进 Blob 里的。而 Blob 是一种类似引用的东西，因此由同一个带有 `Collider` 的 Prefab 实例化出的 Entity，它们的 `CollisionFilter` 也是相同的。如果想要改变，必须要为每个实例化的 Entity 单独创建一个 `Collider` 的 Blob 资源来让它们的 `CollisionFilter` 不同。这就非常蛋疼
...
1.  使用 `CollisionFilter` 来实现的话，碰撞处理过程中的性能要比注入 `IBodyPairsJob` 好；但是使用 `IBodyPairsJob` 不需要考虑上述 `CollisionFilter` 的诸多问题，也避免了销毁与创建的开销；
