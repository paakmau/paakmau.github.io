...
稍微说明一下，VkSurfaceFormatKHR 结构体包含 format 和 colorSpace 两个成员。format 成员表示颜色通道和类型，例如最常见的 VK\_FORMAT\_B8G8R8A8\_SRGB 格式就意味着每个像素由 B、G、R、A 四个通道组成，每个通道各占 8 位，总共 32 位。而 colorSpace 通过 VK\_COLOR\_SPACE\_SRGB\_NONLINEAR\_KHR 标记来是否支持 SRGB 的颜色空间
