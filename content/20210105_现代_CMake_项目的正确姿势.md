...
实际上头文件并不需要添加在目标中，因为在编译目标时，CMake 只是把这些文件一起传递给编译器，编译器只会编译源文件，会直接忽略掉这些头文件。
但是有个问题是某些 IDE 会读取目标中的文件，如果不把头文件一起添加进去，它们可能会被 IDE 忽略。
因此仍然建议把头文件添加进目标中。
...
还要写一个 `.in` 文件，在里面导入依赖，再引入这个 helloTargets.cmake。
使用 configure_file 命令根据这个 `.in` 文件生成最终的 helloConfig.cmake，才可以安装它。
...
于是我们考虑直接把库的指定版本下载下来。
这是 CMake 原生支持的比较没那么折磨的导入外部库的一种方式了。
