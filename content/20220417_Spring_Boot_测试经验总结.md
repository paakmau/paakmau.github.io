...
但是它实际上存在很多隐患。
根据 [Don’t Use @Transactional in Tests](https://dev.to/henrykeys/don-t-use-transactional-in-tests-40eb)，不应当使用 `@Transactional` 的理由很多：
...
另外显然清上下文只能清内存数据库，如果是写进硬盘的那还是参考之前关于事务处理的章节。
