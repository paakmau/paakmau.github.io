...
POSIX shell 的标准可以参考：<https://pubs.opengroup.org/onlinepubs/9799919799/utilities/V3_chap02.html>。
...
- Field splitting
    ...
    特殊字符不便于输入，修改 `${IFS}` 可以用这种写法：`IFS=$'\b\t\n'`。
...
- `${var:?<cmd>}`：指示错误。
    ...
    如果 `var` 未定义或为空串，将 `<cmd>` 展开结果输出到标准错误流，并以非零值退出脚本。
    如果是交互式 shell 则不会退出。
