...
于是 JRE 就会被解压到 `cd ${HY_WORKSPACE}/bisheng-jre-17.0.14` 目录中。
...
这里由于 HyBench 实现上的一些原因，驱动需要下载 5.1.0 版本。

```sh
cd ${HY_WORKSPACE}
mkdir pkg && cd pkg

wget https://opengauss.obs.cn-south-1.myhuaweicloud.com/5.1.0/arm_2203/openGauss-5.1.0-JDBC.tar.gz
tar -zxf openGauss-5.1.0-JDBC.tar.gz
mv opengauss-jdbc-5.0.0.jar ${HY_WORKSPACE}/hybench/lib
...
其中 `stmt_opengauss.toml` 指定了负载语句；`og.props` 指定了并发量与执行时间。
